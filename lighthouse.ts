...
const main = () => {
  program
    .name("lighthouse")
    .description(
      "Lighthouse is a tool for creating NFT collections on the SEI blockchain."
    )
    .version("0.3.6");
      ...
      let wallet = await inquirer.prompt([
        {
          ...
          name: "wallet",
          message:
            "What is the mnemonic keyphrase of the address you want to use in lighthouse?",
        },
      ]);
      ...
      let network = await inquirer.prompt([
        {
          ...
          name: "network",
          message:
            "What is the network you want to use? (pacific-1 is the mainnet, atlantic-2 is the testnet)",
  ...
  program
    ...
    .action(() => {
      let isConfig = fs.existsSync(configFile);
          ...
          .prompt([
            {
              ...
              name: "overwrite",
              message:
                "A config.json file already exists. Do you want to overwrite it?",
              default: false,
            },
          ])
          .then((answers) => {
      ...
      for (let file of files) {
        try {
          let metadata = JSON.parse(
            fs.readFileSync("./assets/" + file, "utf-8")
          );
          if (!metadata.image) {
  ...
  program
    ...
    .action((file, wallet) => {
      ...
      const proof = tree
        .getProof(Buffer.from(keccak_256(wallet)))
        .map((element) => element.data.toString("hex"));

      console.log(
        "verification: " +
          tree.verify(proof, Buffer.from(keccak_256(wallet)), merkleRoot)
      );
  ...
  program
    ...
    .action(async (collection, groupName, answers) => {
      ...
      const client = await SigningCosmWasmClient.connectWithSigner(
        ...
        wallet,
        {
          gasPrice: GasPrice.fromString(
            answers.gasPrice ? answers.gasPrice + "usei" : "0.1usei"
          ),
        }
      );
      ...
      let collectionConfig = await client.queryContractSmart(
        getLighthouseContract(config.network),
      ...
      let recipient = await inquirer.prompt([
        {
          ...
          name: "recipient",
          message:
            "Enter recipient address (default: " + firstAccount.address + ")",
        },
      ]);
      ...
      if (group.merkle_root !== "" && group.merkle_root !== null) {
        ...
        merkleProof = proofArray.map((p: string) =>
          Array.from(Buffer.from(p, "hex"))
        );

        hashedAddress = Array.from(
          Buffer.from(
            keccak_256(
              recipient.recipient ? recipient.recipient : firstAccount.address
            )
          )
        );
      }
      ...
      const coins = [
        {
          denom: "usei",
          amount: new BigNumber(group.unit_price)
            .plus(new BigNumber(lighthouseConfig.fee))
            .toString(),
        },
      ];

      const mintReceipt = await client.execute(
      ...
      console.log(
        "Transaction hash: " + chalk.green(mintReceipt.transactionHash)
      );
  ...
  program
    ...
    .argument("<collection>")
    .argument("<group_name>", "Mint from a specific group")
    .option("-q, --quantity, <quantity>", "Desired quantity")
    .option("-d, --delay", "Delay time or not")
    .option("-t, --timestamp, <time_stamp>", "Timestamp of mint")
    .option(
      "--gas-price <gas_price>",
      "Gas price to use for transaction (default: 0.1)"
    )
    .action(async (collection, groupName, answers) => {
      if (answers.quantity) {
        console.log("Desired quantity: " + answers.quantity);
      }
      ...
      let numMint = answers.quantity ? answers.quantity : recipients.length;
      ...
      for (let i = 0; i < numMint; ++i) {
        if (group.merkle_root !== "" && group.merkle_root !== null) {
          ...
          hashedAddress = Array.from(Buffer.from(keccak_256(recipients[i])));
        }
        let spinner = ora("Minting NFT of " + recipients[i]).start();
        const mintMsg = {
          mint_native: {
            collection,
            group: groupName,
        ...
        spinner.succeed("NFT minted - account: " + chalk.green(recipients[i]));
        console.log(
          "Transaction hash: " + chalk.green(mintReceipt.transactionHash)
        );
        const events = mintReceipt.logs[0].events;
          ...
          let groups = result.mint_groups.map((group: any) => {
            return {
              ...
              merkle_root: group.merkle_root
                ? Buffer.from(group.merkle_root).toString("hex")
                : null,
              max_tokens: group.max_tokens,
              unit_price: new BigNumber(group.unit_price)
                .dividedBy(new BigNumber(1e6))
                .toString(),
              start_time: group.start_time
                ? new Date(group.start_time * 1000).toISOString()
                : null,
              end_time: group.end_time
                ? new Date(group.end_time * 1000).toISOString()
                : null,
  ...
  program
    ...
    .action(async (collection, options) => {
        ...
        update_collection: {
          ...
          mint_groups: config.groups.map((group: any) => {
            return {
              name: group.name,
              merkle_root: group.merkle_root
                ? Array.from(Buffer.from(group.merkle_root, "hex"))
                : null,
              max_tokens: group.max_tokens ? group.max_tokens : 0,
              unit_price: new BigNumber(group.unit_price.toString())
                .times(new BigNumber(1e6))
                .toString(),
              creators: group.creators ? group.creators : [],
              start_time: group.start_time
                ? new Date(group.start_time).getTime() / 1000
                : 0,
              end_time: group.end_time
                ? new Date(group.end_time).getTime() / 1000
                : 0,
            };
          }),
        },
      ...
      const updateReceipt = await client.execute(
        firstAccount.address,
        getLighthouseContract(config.network),
        updateMsg,
        "auto"
      ...
      console.log(
        "Transaction hash: " + chalk.green(updateReceipt.transactionHash)
      );
    });
  ...
  program
    ...
    .option(
      "--code <code_id>",
      "Register already deployed CW721 contract (optional)"
    )
    .option(
      "--gas-price <gas_price>",
      "Gas price to use for transaction  (default: 0.1)"
    ...
    .action(async (answers) => {
      ...
      if (codeId === null) {
        ...
        const contractPath = path.join(
          __dirname,
          "./cw2981_lighthouse_edition.wasm"
        );
        ...
        spinner.succeed(
          "CW721 Contract deployed to SEI blockchain. Contract Code ID: " +
            chalk.green(codeId.toString())
        );
      } else {
        codeId = parseInt(codeId);
        console.log(
          "Using existing CW721 contract with code ID: " +
            chalk.green(codeId.toString())
        );
      }
      ...
      const registerMsg = {
        register_collection: {
          ...
          hidden_metadata: config.hidden_metadata
            ? config.hidden_metadata
            : false,
          placeholder_token_uri: config.placeholder_token_uri
      ...
      const registerReceipt = await client.execute(
        firstAccount.address,
        getLighthouseContract(config.network),
        registerMsg,
        "auto"
      ...
      console.log(
        "Transaction hash: " + chalk.green(registerReceipt.transactionHash)
      );
    ...
    .action(async (collection, metadata, options) => {
      ...
      const Msg = {
        update_reveal_collection_metadata: {
          collection,
          placeholder_token_uri: metadata,
        },
      };
  ...
  program
    ...
    .action(async (collection, token_ids) => {
      ...
      console.log(
        minters
          .map((minter: any) => minter.token_id + " " + minter.minter)
          .join("\n")
      );
  ...
  program
    ...
    .action(async (collection) => {
      ...
      const client = await SigningCosmWasmClient.connect(config.rpc);
      let collectionData = await client.queryContractSmart(
        getLighthouseContract(config.network),
        { get_collection: { collection } }
      );
        ...
        .then((result) => {
          ...
          if (result_type === "full") {
            for (let i = 0; i < result.collections.length; i++) {
              let groups = result.collections[i].mint_groups.map(
                (group: any) => {
                  return {
                    name: group.name,
                    merkle_root: group.merkle_root
